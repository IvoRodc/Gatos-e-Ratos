...
breed[mice mouse]
turtles-own [energia]
mice-own [gato-avistado] ;;variável booleana de ajuda na comunicação entre ratos
...
to setup-agents
  ...
  [
    ...
    let x one-of patches with [pcolor != yellow AND pcolor != red]
    move-to x
  ...
  [
    set shape "cat"
    set color black
    let x one-of patches with [not any? mice-here and not any? mice-on neighbors and not any? cats-here AND pcolor != yellow AND pcolor != red]
...
to setup-cheese
  ask patches with [pcolor != yellow AND pcolor != red AND [pcolor] of neighbors != yellow] ;;tentar que os vizinhos nao sejam queijo também
  [
...
to setup-traps
  ask patches with [pcolor != red AND pcolor != yellow AND [pcolor] of neighbors != red] ;;tentar que os vizinhos nao sejam armadilhas também
  [
...
to go
  ...
  starvation
  tick
  if count mice = 0 [export-final set numero-exp numero-exp + 1 stop] ;;terminar a exportação dos dados
end
  ...
  [
    ...
    [;;ELSE
      ...
      [;;CAÇA-FUGA
        ...
        ifelse any? cats-on patch-left-and-ahead 45 1 OR any? cats-on patch-ahead 1 OR any? cats-on patch-right-and-ahead 45 1 [fd (-1)]
        [;;ELSE FRENTE
          ifelse any? cats-on patch-left-and-ahead 135 1 OR any? cats-on patch-ahead (-1) OR any? cats-on patch-right-and-ahead 135 1 [fd 1]
          [;;ELSE TRÁS
            ifelse any? cats-on patch-left-and-ahead 90 1 [move-to patch-right-and-ahead 90 1]
            [;;ELSE ESQUERDA
              ifelse any? cats-on patch-right-and-ahead 90 1 [move-to patch-left-and-ahead 90 1]
              [;;ELSE DIREITA
                ;;se o rato estiver perto de um gato, tentar comunicar a qualquer rato proximo, para se afastar
                if any? cats-on neighbors []
                [;;ELSE -> NAO HA GATOS POR PERTO
      ...
      [;;ELSE
        ifelse experiencia = "Caça-Fuga and Cheese-Traps"
...
to print-header
  ...
  [
    ...
    [
      print-lb ";"
      print-nlb "EXPERIENCIA" print-nlb ";" print-nlb experiencia print-lb ";"
      print-lb ";"
      ...
      print-lb ";"
      print-nlb "Numero de Cats" print-nlb ";" print-nlb "Numero de Mice" print-nlb ";" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;" print-nlb "Ticks;"
      print-lb ""
    ]
  ]
...
to export-final
    ifelse numero-exp = 1
    [;;THEN
      print-nlb N-cats print-nlb ";" print-nlb N-mice print-nlb ";" print-nlb ticks print-nlb ";"
    ]
    [;;ELSE
      print-nlb ticks print-nlb ";"
    ]
